...
camera_names_real = ["cam_high", "cam_low", "cam_left_wrist", "cam_right_wrist"]
...
def load_episode_to_data(dataset_path):
    ...
    with h5py.File(dataset_path, "r") as root: 
        ...
        all_cam_images = []
        for cam_name in camera_names_real:
    ...
    episode = {
        ...
        "cam_high": all_cam_images[0].squeeze(),  # [T, H, W, C]
        "cam_low": all_cam_images[1].squeeze(),  # [T, H, W, C]
        "cam_left_wrist": all_cam_images[2].squeeze(),  # [T, H, W, C]
        "cam_right_wrist": all_cam_images[3].squeeze(),  # [T, H, W, C]
        "qpos": qpos,  # [T, 14]
...
def get_data(data, idx):
    ...
    for key in rgb_keys:
        ...
        obs_dict[key] = np.expand_dims(obs_dict[key], axis=0) 
...
@click.option('-dv', '--device', default='cuda:1', help="Device to run the evaluation on")
def main(keypose_ckpt, dataset_dir, task, epi_num, model_type, step_num, device):
