...
def _find_keypose_idx_hang(
    ...
    if side == 'left':
        for i in range(375, 625):  # 7.5-12.5s
            if (
                gripper[i-2] >= 0.5 * gripper_epsilon and
                gripper[i-1] >= 0.5 * gripper_epsilon and
                gripper[i] < 0.5 * gripper_epsilon
            ):
                keypose_indices.append(i)
                break

    return keypose_indices, 0, False
...
def _plot_ee_and_gripper(
    ...
    plt.tight_layout()
    plt.savefig(f'{img_dir}/episode_{i}_ee.png', dpi=100)
    plt.close()


def _remove_too_close_keyposes(keypose_indices: np.ndarray, min_dist=5, forward=False):
    if forward:
