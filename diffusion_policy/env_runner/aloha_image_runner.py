...
class AlohaImageRunner(BaseImageRunner):
    def __init__(
        ...
        ## fixed 3 seed sets
        seed_num = 3
        seed_set = [test_start_seed + i for i in range(seed_num)]

        for i in range(n_test):
            ## random 3 seeds
            # seed = random.choice(seed_set)
            ## each seed for each test_env
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                keypose_tgt = np_action_dict["tgt_keypose"]
                Ta = action_seq.shape[1]
                keypose_tgt = np.tile(keypose_tgt[:, None, :], (1, Ta, 1))
                action = np.concatenate([action_seq, keypose_tgt], axis=-1) ## for ploting
        ...
        for i in range(len(self.env_fns)):
            ...
            log_data[prefix + f"sim_max_reward_{i}_{seed}"] = max_reward
        ...
        ## min distance to target keypose
        log_data["min_distance_to_target"] = policy.min
        log_data["seeds"] = list(set(self.env_seeds))
