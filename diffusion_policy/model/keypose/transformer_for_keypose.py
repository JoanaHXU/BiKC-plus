...
from diffusion_policy.model.keypose.position_embedding import PositionEmbedding2D
...
class TransformerForKeypose(ModuleAttrMixin):
    ...
    def __init__(self,
        ...
        qpo_emb = nn.Parameter(torch.zeros(1, d_embedding))
        keypose_emb = nn.Parameter(torch.zeros(1, d_embedding))
        ...
        self.qpo_emb = qpo_emb
        self.keypose_emb = keypose_emb
        ...
        assert len(low_dim_keys) == 2, f"low_dim_keys should be (qpo, keypose), but now is {low_dim_keys}"
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            torch.nn.init.normal_(module.qpo_emb, mean=0.0, std=0.02)
            torch.nn.init.normal_(module.keypose_emb, mean=0.0, std=0.02)
    ...
    def get_optim_groups(self, weight_decay: float=1e-3):
        ...
        no_decay.add("qpo_emb")
        no_decay.add("keypose_emb")
    ...
    def forward(self,
        ...
        @return tgt_keypose: (B, output_dim)
        """
        ...
        qpo_features = obs_feature["qpos"] + self.qpo_emb
        keypose_features = obs_feature["keypose"] + self.keypose_emb
        qpo_features = qpo_features.unsqueeze(1)
        keypose_features = keypose_features.unsqueeze(1)
        
        # obs feature sequence (B, (ncam h w) + 2, D) := (B, L, D)
        context_seq = torch.cat(
            [rgb_features, qpo_features, keypose_features],
            dim=1
        )
        ...
        ### output projection
        x = self.keypose_ln(x)
        tgt_keypose = self.keypose_head(x)
...
if __name__ == "__main__":
    ...
    example_input = {
        ...
        "keypose": torch.randn(2, 256)
    }

    tgt_keypose = transformer(example_input)

    print(tgt_keypose.shape)  # (2, 14)
